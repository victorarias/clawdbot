...
function resolveElevatedPermissions(params: {
  ...
  const normalizedChannel = normalizeChannelId(params.provider);
  const dockFallbackAllowFrom = normalizedChannel
    ? getChannelDock(normalizedChannel)?.elevated?.allowFromFallback?.({
        cfg: params.cfg,
